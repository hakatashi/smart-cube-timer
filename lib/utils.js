...
const getRotationMatrixFromFaces = ({from: [from1, from2], to: [to1, to2]}) => {
	...
	if (fromVector1.isParallelTo(toVector1) || fromVector2.isParallelTo(toVector2)) {
		...
		assert(fromProjection.dot(axis) <= Number.EPSILON);
		assert(toProjection.dot(axis) <= Number.EPSILON);

		const angle = toProjection.angleFrom(fromProjection);
	...
	const angle = toProjection1.angleFrom(fromProjection1);
	// Why should I invert angle?
...
const getRotationFromFaces = ({from: [from1, from2], to: [to1, to2]}) => {
	...
	// decomposit matrix into rotations around x, y, z (order: z => x => y)
	// https://qiita.com/q_tarou/items/46e5045068742dfb2fa6
	if (eq(matrix.e(3, 2), 1)) {
		return {
			x: Math.PI / 2,
			y: 0,
	...
	return {
		x: Math.asin(matrix.e(3, 2)),
		y: Math.atan2(-matrix.e(3, 1), matrix.e(3, 3)),
...
export const getRotationNotationFromFaces = ({from: [from1, from2], to: [to1, to2]}) => {
	...
	if (!eq(rotation.z, 0)) {
		const amount = Math.round(rotation.z / Math.PI * 2);
		notations.push(getNotation({face: 'y', amount: (amount + 5) % 4 - 1}));
	}

	if (!eq(rotation.x, 0)) {
		const amount = Math.round(rotation.x / Math.PI * 2);
		notations.push(getNotation({face: 'x', amount: (amount + 5) % 4 - 1}));
	}

	if (!eq(rotation.y, 0)) {
		const amount = -Math.round(rotation.y / Math.PI * 2);
		notations.push(getNotation({face: 'z', amount: (amount + 5) % 4 - 1}));
	}
