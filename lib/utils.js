...
import {olls, plls, clls} from '~/lib/data.js';
...
export const isRouxBlockSatisfied = ({cube, face, bottom, bottomDirection}) => {
	if (face === bottom || getOppositeFace(face) === bottom) {
		return false;
	}
...
export const isRouxStageSatisfied = ({cube, stage, rouxBlock: {side, bottom}}) => {
	...
	const {eo, co} = getFaceOrientations(top, cube, topDirection);
	const {ep, cp} = getFacePermutations(top, cube, topDirection);

	const cll = getCll({co, cp});

	if (stage === 'cll') {
		return {
			result: cll.name === 'CLL Skip',
			bottomDirection,
			cll,
		};
	}
