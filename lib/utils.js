...
const getRotationMatrixFromFaces = ({from: [from1, from2], to: [to1, to2]}) => {
	...
	// special case where from1, from2, to1, to2 all reside in the same plane (case 1)
	if (fromVector1.add(toVector2).eql(fromVector2.add(toVector1))) {
		const fromVector = fromVector1.add(fromVector2);
		const toVector = toVector1.add(toVector2);
		const vector1 = fromVector1.add(toVector1);
		const vector2 = fromVector2.add(toVector2);
		const axis = vector1.subtract(vector2).toUnitVector();

		assert(eq(axis.modulus(), 1));
		assert(eq(fromVector.dot(axis), 0));
		assert(eq(toVector.dot(axis), 0));

		const angle = fromVector.cross(toVector).isParallelTo(axis)
			? toVector.angleFrom(fromVector)
			: -toVector.angleFrom(fromVector);
		assert(fromVector.rotate(angle, Line.create([0, 0, 0], axis)).eql(toVector));
	...
	// special case where from1, from2, to1, to2 all reside in the same plane (case 2)
	if (fromVector1.add(fromVector2).eql(toVector1.add(toVector2))) {
		const fromVector = fromVector1.subtract(fromVector2);
		const toVector = toVector1.subtract(toVector2);
		const axis = fromVector.cross(toVector).toUnitVector();

		// get vector projection of from and to onto axis
		// https://en.wikipedia.org/wiki/Vector_projection
		assert(axis.modulus() === 1);
		const fromProjection = fromVector1.subtract(axis.multiply(fromVector1.dot(axis)));
		const toProjection = toVector1.subtract(axis.multiply(toVector1.dot(axis)));
		assert(eq(fromProjection.dot(axis), 0));
		assert(eq(toProjection.dot(axis), 0));
		...
		assert(fromVector2.rotate(angle, Line.create([0, 0, 0], axis)).eql(toVector2));
...
export const getRelativeFaceFromFaces = (face, {from: [from1, from2], to: [to1, to2]}) => {
	const matrix = getRotationMatrixFromFaces({from: [from1, from2], to: [to1, to2]});
	const faceVector = Vector.create(faceVectors[face]);
