...
const permutations = Array(24).fill().map((_, i) => {
	const items = [0, 1, 2, 3];
	const permutation = [];
	let reminder = i;
	for (const j of Array(4).keys()) {
		permutation.push(items[reminder % (4 - j)]);
		items.splice(reminder % (4 - j), 1);
		reminder = Math.floor(reminder / (4 - j));
	}
	return permutation;
});

const cornerOrientations = Array(3 ** 4).fill().map((_, i) => {
	let reminder = i;
	let orientation = [];
	for (const j of Array(4).keys()) {
		orientation.push(reminder % 3);
		reminder = Math.floor(reminder / 3);
	}
	return orientation;
});

const edgeOrientations = Array(2 ** 4).fill().map((_, i) => {
	let reminder = i;
	let orientation = [];
	for (const j of Array(4).keys()) {
		orientation.push(reminder % 2);
...
const ollCache = new Map();
for (const eo of edgeOrientations) {
	for (const co of cornerOrientations) {
		ollCache.set(eo.join('') + co.join(''), getOll({eo, co}));
	}
}

const getCachedOll = ({eo, co}) => (
	ollCache.get(eo.join('') + co.join(''))
);

export {getCachedOll as getOll};
...
const pllCache = new Map();
for (const ep of permutations) {
	for (const cp of permutations) {
		pllCache.set(ep.join('') + cp.join(''), getPll({ep, cp}));
	}
}

const getCachedPll = ({ep, cp}) => (
	pllCache.get(ep.join('') + cp.join(''))
);

export {getCachedPll as getPll};
