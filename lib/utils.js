...
const getFaceOrientations = (face, cube) => {
	...
	const co = faceCorners.map(([cornerIndex, cornerPiece]) => (
		(
			3 +
			[...corners[cube.cp[cornerIndex]]].findIndex((f) => f === face) -
	...
	const eo = faceEdges.map(([edgeIndex, edgePiece]) => (
		(
			2 +
			[...edges[cube.ep[edgeIndex]]].findIndex((f) => f === face) -
...
const getFacePermutations = (face, cube) => {
	...
	console.log({faceEdges, faceCorners, edges, corners, cubeEp: cube.ep, cubeCp: cube.cp, ep, cp, face});
...
export const isCfopStageSatisfied = ({cube, stage, cross}) => {
	...
	console.log({cross, cal: getOppositeFace(cross), oppositeFace, cube});
...
export const isRouxStageSatisfied = ({cube, stage, base}) => {

};

export const isStageSatisfied = ({mode, cube, stage, cross, base}) => {
	if (mode === 'cfop') {
		return isCfopStageSatisfied({cube, stage, cross});
	}

	assert(mode === 'roux');
	return isRouxStageSatisfied({cube, stage, base});
};
