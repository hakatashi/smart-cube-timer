	...
	toText({cross = null, rouxBlock = null, fixDirection = false, slices = []} = {}) {
		...
		if (rouxBlock === null) {
			const cubeOrientation = cross === null ? {
				left: 'L',
				down: 'D',
			} : getRotationOrientation({from: cross, to: 'D'});
			...
			for (const move of this.moves) {
				...
				if (
					...
					const slice = slices.find((slice) => newMove.face === sliceFaces[slice] || lastMove.face === sliceFaces[slice]);
					...
					if (slice !== undefined) {
						...
						const newMoves = [
							{...lastMove, amount: lastMove.amount + amount * (Math.sign(lastMove.amount) === direction ? -1 : 1)},
							{face: slice, amount},
							{...newMove, amount: newMove.amount - amount * (Math.sign(lastMove.amount) === direction ? -1 : 1)},
						];

						moves.pop();
						moves.push(...newMoves.filter(({amount}) => amount !== 0));
